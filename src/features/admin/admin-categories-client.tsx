    ...
    <div>
      ...
      {showCreate ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-xl border bg-white p-6 shadow-lg" style={{ borderColor: C.cardBorder }}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold" style={{ color: C.textPrimary }}>New category</h2>
              <button onClick={() => setShowCreate(false)} className="text-sm" style={{ color: C.textSecondary }}>Cancel</button>
            </div>
            <form
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="grid gap-1">
                  <span className="text-xs font-medium" style={{ color: C.textSecondary }}>Sort order</span>
      ...
      {editingCategory ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-xl border bg-white p-6 shadow-lg" style={{ borderColor: C.cardBorder }}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold" style={{ color: C.textPrimary }}>Edit category</h2>
              <button onClick={() => setEditingId(null)} className="text-sm" style={{ color: C.textSecondary }}>Cancel</button>
            </div>
            <form
              action={upsertCategoryAction}
            ...
            >
              ...
              <label className="grid gap-1">
                <span className="text-xs font-medium" style={{ color: C.textSecondary }}>Label</span>
                <input name="label" defaultValue={editingCategory.label} required
                  className="min-h-10 rounded-lg border px-3 text-sm outline-none focus:ring-2"
                  style={{ borderColor: C.cardBorder, color: C.textPrimary }} />
              </label>
              <label className="grid gap-1">
                <span className="text-xs font-medium" style={{ color: C.textSecondary }}>Description</span>
                <textarea name="description" rows={3} defaultValue={editingCategory.description ?? ""}
                  className="rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2"
                  style={{ borderColor: C.cardBorder, color: C.textPrimary }} />
              </label>
              <div className="grid grid-cols-2 gap-3">
                ...
                <label className="grid gap-1">
                  <span className="text-xs font-medium" style={{ color: C.textSecondary }}>Status</span>
                  <select name="is_active" defaultValue={String(editingCategory.is_active)}
                    className="min-h-10 rounded-lg border px-3 text-sm outline-none focus:ring-2"
                    style={{ borderColor: C.cardBorder, color: C.textPrimary }}>
                    <option value="true">Active</option>
                    <option value="false">Inactive</option>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit"
                  className="min-h-10 rounded-lg px-4 text-sm font-medium text-white transition-all hover:opacity-90"
                  style={{ backgroundColor: C.accent }}>Save changes</button>
              </div>
            </form>
            <form action={deleteCategoryAction} className="mt-3">
              ...
              <button type="submit"
                ...
                style={{ backgroundColor: C.error }}>Delete category</button>
            </form>
          </div>
        </div>
      ) : null}
        ...
        <table className="w-full text-sm" style={{ color: C.textPrimary }}>
          <thead>
            <tr style={{ backgroundColor: "#f8fafc" }}>
              <th className="px-4 py-3 text-left font-medium text-xs uppercase tracking-wider" style={{ color: C.textSecondary }}>ID</th>
              <th className="px-4 py-3 text-left font-medium text-xs uppercase tracking-wider" style={{ color: C.textSecondary }}>Label</th>
              <th className="px-4 py-3 text-left font-medium text-xs uppercase tracking-wider" style={{ color: C.textSecondary }}>Status</th>
              <th className="px-4 py-3 text-right font-medium text-xs uppercase tracking-wider" style={{ color: C.textSecondary }}>Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y" style={{ borderColor: C.cardBorder }}>
            {categories.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-sm" style={{ color: C.textSecondary }}>
                  No categories yet. Click &quot;Add category&quot; to create one.
                </td>
              </tr>
            ) : (
              categories.map((cat) => (
                <tr key={cat.id} className="transition-colors hover:bg-[#f8fafc]">
                  <td className="px-4 py-3 font-mono text-xs" style={{ color: C.textSecondary }}>{cat.id}</td>
