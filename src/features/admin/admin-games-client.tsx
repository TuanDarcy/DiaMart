  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{ color: C.textPrimary }}>Games</h1>
          <p className="text-sm" style={{ color: C.textSecondary }}>{games.length} games</p>
        </div>
      ...
      {showCreate ? (
        <Modal onClose={() => setShowCreate(false)} title="New game">
          <GameForm action={upsertGameAction} onDone={() => setShowCreate(false)} />
        </Modal>
      ) : null}
      ...
      {editGame ? (
        <Modal onClose={() => setEditingId(null)} title="Edit game">
          <GameForm action={upsertGameAction} game={editGame} onDone={() => setEditingId(null)} />
          <form action={deleteGameAction} className="mt-3">
            <input name="id" type="hidden" value={editGame.id} />
            <button type="submit" className="min-h-10 w-full rounded-lg px-4 text-sm font-medium text-white transition-all hover:opacity-90"
              style={{ backgroundColor: C.error }}>Delete game</button>
          </form>
        </Modal>
      ) : null}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ backgroundColor: "#f8fafc" }}>
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: C.textSecondary }}>Name</th>
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: C.textSecondary }}>Slug</th>
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: C.textSecondary }}>Image</th>
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider" style={{ color: C.textSecondary }}>Status</th>
              <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider" style={{ color: C.textSecondary }}>Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y" style={{ borderColor: C.cardBorder }}>
            {games.map(g => (
              <tr key={g.id} className="transition-colors hover:bg-[#f8fafc]">
                <td className="px-4 py-3 font-medium" style={{ color: C.textPrimary }}>{g.name}</td>
                <td className="px-4 py-3" style={{ color: C.textSecondary }}>{g.slug}</td>
                <td className="px-4 py-3">
                  {g.image_src ? (
                    <img src={g.image_src} alt={g.image_alt ?? g.name} className="h-10 w-10 rounded-lg object-cover" />
                  ) : (
...
function GameForm({ action, game, onDone }: {
    ...
    <form action={action} onSubmit={() => setTimeout(onDone, 100)} className="grid gap-4">
      ...
      <label className="grid gap-1">
        ...
        <p className="text-xs" style={{ color: C.textSecondary }}>
          Paste a .png or .jpg URL. Use free image hosting like imgur, postimages, or paste direct CDN links.
        </p>
      </label>

      <Field name="image_alt" label="Image alt text" defaultValue={game?.image_alt} />
      <div className="grid grid-cols-2 gap-3">
        <Field name="sort_order" label="Sort order" type="number" defaultValue={game?.sort_order ?? 0} />
        <label className="grid gap-1">
          <span className="text-xs font-medium" style={{ color: C.textSecondary }}>Status</span>
          <select name="is_active" defaultValue={String(game?.is_active ?? true)}
            className="min-h-10 rounded-lg border px-3 text-sm outline-none focus:ring-2"
            style={{ borderColor: C.cardBorder, color: C.textPrimary }}>
      ...
      <label className="grid gap-1">
        <span className="text-xs font-medium" style={{ color: C.textSecondary }}>Description</span>
        <textarea name="description" rows={3} defaultValue={game?.description ?? ""}
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button type="submit" className="min-h-10 rounded-lg px-4 text-sm font-medium text-white transition-all hover:opacity-90"
          style={{ backgroundColor: C.accent }}>{game ? "Save changes" : "Create game"}</button>
      </div>
    </form>
  );
}
